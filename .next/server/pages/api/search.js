...
module.exports = __webpack_require__("j0nM");
...
/* harmony default export */ __webpack_exports__["default"] = (async function (req, res) {
  try {
    const results = await Object(_db__WEBPACK_IMPORTED_MODULE_0__[/* queryDB */ "c"])(`SELECT * FROM
                (SELECT DISTINCT ON
                    (id) id,
                    title,
                    category,
    ...
    const authorArticleIDs = await Object(_db__WEBPACK_IMPORTED_MODULE_0__[/* queryDB */ "c"])(`SELECT articles FROM authors WHERE LOWER(name) LIKE LOWER('%${req.body.value}%')`);
    const authorResults = await Object(_db__WEBPACK_IMPORTED_MODULE_0__[/* queryDB */ "c"])('SELECT * FROM articles WHERE id = ANY($1)', [authorArticleIDs]); // console.log(JSON.stringify(results));
