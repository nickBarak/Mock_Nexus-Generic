...
function AboutPage() {
	...
	return (
		<Layout>
			<About heading="About">
				<div>
					Lor <span style={{ fontStyle: 'italic' }}>Mock Nexus</span>{' '}
					{lipsum.slice(lipsumCount.current, lipsumCount.current += `had its start in the 1930s. Back then, the newspaper was
					called the`.length)}{' '}
					<span style={{ fontStyle: 'italic' }}>Albatross</span>, {lipsum.slice(lipsumCount.current, lipsumCount.current += `and UC
					...
					Mal <span style={{ fontStyle: 'italic' }}>Mock Nexus</span>{' '}
					{lipsum.slice(lipsumCount.current, lipsumCount.current += `received its name in 1970, following the infamous Bank of
					America burning that occurred in Isla Vista`.length)}.
					<br />
					<br />
					{lipsum.slice(lipsumCount.current, lipsumCount.current += `The editorial board took the name from a Robert Maynard
					Hutchins quote`.length)}: “{lipsum.slice(lipsumCount.current, lipsumCount.current += `A free press is the nexus of any
					democracy`.length)}.”
					<br />
					<br />
					Vor <span style={{ fontStyle: 'italic' }}>Nexus</span>{' '}
					{lipsum.slice(lipsumCount.current, lipsumCount.current += `continues to be the go-to source to stay up-to-date on all
					...
					<span style={{ fontStyle: 'italic' }}>Nexus</span> {lipsum.slice(lipsumCount.current, lipsumCount.current += `provides
					...
					{lipsum.slice(lipsumCount.current, lipsumCount.current += `is also an award-winning media source, ranked in the top-10
					...
					<span style={{ fontStyle: 'italic' }}>Nexus</span> {lipsum.slice(lipsumCount.current, lipsumCount.current += `also
				...
				<div style={{ marginTop: '2rem', marginLeft: '.5rem' }}>
					...
					<div style={{ fontSize: '.85rem', marginTop: '3px' }}>
						...
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `newsroom, located underneath Storke Tower`.length)}.
						<label style={{ fontStyle: 'italic' }}>
							{' '}
							Boriali Nullempo/Mock Nexus
						</label>
					</div>
				</div>
				<div style={{ marginTop: '2rem' }}>
					...
					<div>
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `Here at the`.length)}{' '}
						<span style={{ fontStyle: 'italic' }}>Mock Nexus</span>{' '}
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `we recognize the importance of free speech and encourage
						...
						<br />
						<br />
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `Comments may be removed if these guidelines are not met
						...
						<br />
						<br />
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `Comments may be removed or turned off for an article if
						there are concerns regarding an individual’s safety`.length)}.
						<br />
						<br />
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `As of May 2020, readers must include their name and
						email address in order to post a comment`.length)}.
						<br />
						<br />{lipsum.slice(lipsumCount.current, lipsumCount.current += `A comment will be removed if it`.length)}:
				...
				<div style={{ marginTop: '2rem' }}>
					...
					<div>
						{lipsum.slice(lipsumCount.current, lipsumCount.current += `In articles written by "Mock Nexus Editorial Board,"
